...
> 다른 환경에서도 git pull 하면 그대로 읽힙니다. 최종 갱신: 2026-08-12.

---

## 작업 방식 / 피드백 (반드시 준수)

- **서브에이전트 리포트(QA/code-reviewer)는 팩트 체크 후 필터링 (2026-08-12 확정)**: 서브에이전트가 낸 "버그/취약점" 지적을 사용자에게 그대로 전달 금지. (1) 인용된 file:line은 반드시 Read로 실제 확인 (2) "버그" 단정된 항목은 `plans/`, `CLAUDE.md 현재 개발 단계`, `mockups/`로 설계 의도 검증 (3) CLAUDE.md "impossible scenario 방어 금지" 원칙에 저촉되는 지적은 필터. **동시에 리포트를 "오지적"으로 반박할 때도 반드시 팩트 체크**: 라이딩 코스 2차 사이클에서 code-reviewer가 `Icons.thumb_up` 지적한 걸 Claude가 "리뷰어 오지적"이라 판단했으나 실제 git baseline은 정말 `thumb_up`이었음(현재 상태 잘못 읽음). QA "START/GOAL 드래그 = 버그" 지적은 설계 의도(사용자가 코스 방향 뒤집기용)를 몰라서 나온 오판. 반박 근거도 실제 코드/스펙 확인 후 판단할 것. `.claude/agents/qa.md`와 `code-reviewer.md`에 사전 조사 5단계 + 팩트 체크 규칙 강화됨.

...
- **스코프 아웃 확정 (다시 이월 금지)**:
  ...
  - 카카오맵 딥링크 — 2026-08-12 사용자 결정 (네이버 지도만 유지)
  - `FuelingServiceTest` 복구 — 2026-08-12 사용자 결정
  - 뱅킹 세션 서버 백업 — 2026-08-12 로컬 전용 확정
  - **오프라인 시나리오 검증·강화 — 2026-08-12 사용자 결정**: 대부분 상황이 온라인이라는 전제. 오프라인은 "어쩔 수 없이 못 쓰는" 상태로 수용. 기존 Phase 3 로컬 우선 아키텍처(bike/fueling/maintenance/뱅킹 로컬 저장 + 온라인 시 sync)는 그대로 유지하되, 추가 오프라인 UX 강화·실기기 오프라인 회귀 시나리오 검증 등은 하지 않음
